# Title: ppmi_nCV_DEA_MLclass.R
# Authorship: Elisa Gomez de Lope, Contact details: elisa.gomezdelope@uni.lu
# Info: This script performs nested crossvalidation for feature selection (through DEA) + binary ML classifiers on RNAseq static data (BL).
# Usage (gene level): Rscript ppmi_nCV_DEA_MLclass.R 10 -n yes (for 10 features and normalized counts!)
# Usage (aggregated level): Rscript ppmi_nCV_DEA_MLclass.R 10 -l gobp -s mean 
...
if ((e_level == "GENE") & (norm_counts == "NO")) { 
  ...
  OUT.AUC.FILE <- file.path(OUT_DIR, paste(e_level, paste0(k, "ft"), target, "AUC.tsv", sep = "_"))
  OUT.ERROR.FILE <- file.path(OUT_DIR, paste(e_level, paste0(k, "ft"), target, "ERROR.tsv", sep = "_"))
  OUT.MODEL.FILE <- file.path(OUT_DIR, paste(e_level, paste0(k, "ft"), target, "final_model.rds", sep = "_"))
  OUT.PROBS.FILE <- file.path(OUT_DIR, paste(e_level, paste0(k, "ft"), target, "final_probs.tsv", sep = "_"))
  OUT.FPERF.FILE <- file.path(OUT_DIR, paste(e_level, paste0(k, "ft"), target, "final_performance.tsv", sep = "_"))
  OUT.PLOTS.FILE <- file.path(OUT_DIR_PLOTS, paste(e_level, paste0(k, "ft"), target, "varImp.pdf", sep = "_"))
  OUT.SHAPVALUES.FILE <- file.path(OUT_DIR_PLOTS_SHAP, paste(e_level, paste0(k, "ft"), target, "shap_values.csv", sep = "_"))
  OUT.SHAPDATA.FILE <- file.path(OUT_DIR_PLOTS_SHAP, paste(e_level, paste0(k, "ft"), target, "shap_data.csv", sep = "_"))
  ...
  OUT.AUC.FILE <- file.path(OUT_DIR, paste(e_level, "norm", paste0(k, "ft"), target, "AUC.tsv", sep = "_"))
  OUT.ERROR.FILE <- file.path(OUT_DIR, paste(e_level, "norm", paste0(k, "ft"), target, "ERROR.tsv", sep = "_"))
  OUT.MODEL.FILE <- file.path(OUT_DIR, paste(e_level, "norm", paste0(k, "ft"), target, "final_model.rds", sep = "_"))
  OUT.PROBS.FILE <- file.path(OUT_DIR, paste(e_level, "norm", paste0(k, "ft"), target, "final_probs.tsv", sep = "_"))
  OUT.FPERF.FILE <- file.path(OUT_DIR, paste(e_level, "norm", paste0(k, "ft"), target, "final_performance.tsv", sep = "_"))
  OUT.PLOTS.FILE <- file.path(OUT_DIR_PLOTS, paste(e_level, "norm", paste0(k, "ft"), target, "varImp.pdf", sep = "_"))
  OUT.SHAPVALUES.FILE <- file.path(OUT_DIR_PLOTS_SHAP, paste(e_level, "norm", paste0(k, "ft"), target, "shap_values.csv", sep = "_"))
  OUT.SHAPDATA.FILE <- file.path(OUT_DIR_PLOTS_SHAP, paste(e_level, "norm", paste0(k, "ft"), target, "shap_data.csv", sep = "_"))
  features_varname <- "GENEID"
  
} else { # aggregations
  EXPRS.FILE <- file.path(OUT_DIR_PATHWAY, paste(e_level, st, "expression.tsv", sep = "_"))
  OUT.AUC.FILE <- file.path(OUT_DIR_PATHWAY, paste(e_level, st, paste0(k, "ft"), target, "AUC.tsv", sep = "_"))
  OUT.ERROR.FILE <- file.path(OUT_DIR_PATHWAY, paste(e_level, st, paste0(k, "ft"), target, "ERROR.tsv", sep = "_"))
  OUT.MODEL.FILE <- file.path(OUT_DIR_PATHWAY, paste(e_level, st, paste0(k, "ft"), target, "final_model.rds", sep = "_"))
  OUT.PROBS.FILE <- file.path(OUT_DIR_PATHWAY, paste(e_level, st, paste0(k, "ft"), target, "final_probs.tsv", sep = "_"))
  OUT.FPERF.FILE <- file.path(OUT_DIR_PATHWAY, paste(e_level, st, paste0(k, "ft"), target, "final_performance.tsv", sep = "_"))
  OUT.PLOTS.FILE <- file.path(OUT_DIR_PLOTS, paste(e_level, st, paste0(k, "ft"), target, "varImp.pdf", sep = "_"))
  OUT.SHAPVALUES.FILE <- file.path(OUT_DIR_PLOTS_SHAP, paste(e_level, st, paste0(k, "ft"), target, "shap_values.csv", sep = "_"))
  OUT.SHAPDATA.FILE <- file.path(OUT_DIR_PLOTS_SHAP, paste(e_level, st, paste0(k, "ft"), target, "shap_data.csv", sep = "_"))
...
# Main -------------------------------------------------------------------------
if ((!dir.exists(OUT_DIR)) | (!dir.exists(OUT_DIR_PATHWAY)) | (!dir.exists(OUT_DIR_DATA)) | (!dir.exists(OUT_DIR_PLOTS)) | (!dir.exists(OUT_DIR_PLOTS_SHAP))) {
...
### exporting data for sanity check in python
if (e_level == "GENE") {
...
if (e_level == "GENE") {
  ...
  readr::write_tsv(hout_4ML %>% rownames_to_column(var = "SAMPLE_ID"), file = file.path(OUT_DIR_DATA, paste0(e_level, "_", st, "_data_test_expr_4MLL_", target, ".tsv")))
}
###
...
MODELS = list()
model_list_caret <- c("rf", "glmnet", 'regLogistic', 'LogitBoost', "svmLinear2", 
                      "svmRadial", "svmPoly", "xgbDART", 'xgbLinear', "mlpWeightDecayML") 
...
for (i in 1:ncol(split_index_outer)){
  ...
  error_pred <- as.factor(predict(MODELS[[i]][[model]][[j]], newdata = X_test)) # shouldn't be MODELS[[i]][[model]][["best"]] ??
...
if (("Overall" %in% names(variable_imp$importance)) & (nrow(variable_imp$importance) > 20)) {
  vi <- as.data.frame(variable_imp$importance) %>%
    rownames_to_column(features_varname) %>%
    arrange(desc(Overall)) %>%
    top_n(20) %>%
    mutate_at(vars(all_of(features_varname)), 
              ~gsub("`", "", .)) %>%
